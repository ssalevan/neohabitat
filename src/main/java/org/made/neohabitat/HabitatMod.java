    ...
    public void generic_PUT(User from, Container cont, int pos_x, int pos_y, int obj_orient) {
        ...
        if (cont.noid != THE_REGION) {
            ...
            if (selfClass == CLASS_TOKENS && token_at != -1) {
                pos_y = token_at;
                /*
                 * TODO Merge Tokens objptr = ObjList(cont.contents->c(pos_y));
                 * tvalue = tget(selfptr) + tget(objptr); if (tvalue > 65536)
                 * then goto putFAIL; call tset(selfptr, tvalue); call
                 * b_msg_5(null(), FIDDLE_$, self.noid, C64_TOKEN_denom_offset,
                 * 2, selft.denom_lo, selft.denom_hi); call b_msg_1(null(),
                 * GOAWAY_$, obj.noid); call destroy_object(obj.noid);
                 */
            }
            if (pos_y == -1) {
